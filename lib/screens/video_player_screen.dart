...
import '../mpv/mpv_owned_options.dart';
...
class VideoPlayerScreenState extends State<VideoPlayerScreen>
  ...
  Future<void> _runPlayerInitializationAttempt(int generation, {required bool replaceCurrent}) async {
    ...
    try {
      ...
      // been applied - by mpv's own parser, before initialization, with
      // profiles and their conditions intact. Replaying it through the
      // property API here would flatten those profiles back out, which is the
      // bug the config file exists to fix (#2407).
      final configAppliedByMpv = currentPlayer is PlayerNative && currentPlayer.loadedConfigFile;
      final customMpvConfig = configAppliedByMpv
          ? const <String, String>{}
          : SettingsService.parseMpvConfigText(settingsService.read(SettingsService.mpvConfigText));
      ...
      final ownedHere = PlayerNative.usesLinuxVideoPlane ? appOwnedMpvProperties : appInterceptedMpvProperties;
      for (final entry in customMpvConfig.entries) {
        ...
        if (ownedHere.contains(entry.key)) {
          if (appEmbeddedOwnedMpvProperties.contains(entry.key)) {
            appLogger.w(
